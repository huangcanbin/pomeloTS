...
class HttpUtils():
    ...
    def ExecRequest(self):
        ...
        if self.__request_mode == eRequestType.kHead:
            self.__curl_context.setopt(pycurl.HEADERFUNCTION,
                                       self.OnResponseHeaders)
            self.__curl_context.setopt(pycurl.NOBODY, 1)
        elif self.__request_mode == eRequestType.kGet:
            self.__curl_context.setopt(pycurl.HEADERFUNCTION,
                                       self.OnResponseHeaders)
            self.__curl_context.setopt(pycurl.WRITEFUNCTION,
                                       self.OnResponseContent)
            ...
            self.__curl_context.setopt(pycurl.POSTFIELDSIZE,
                                       len(self.__post_fields))
            self.__curl_context.setopt(pycurl.HEADERFUNCTION,
                                       self.OnResponseHeaders)
            self.__curl_context.setopt(pycurl.WRITEFUNCTION,
                                       self.OnResponseContent)
        ...
        if self.__proxy_flag == 1:
            self.__curl_context.setopt(
                pycurl.PROXY,
                self.__proxy_address + ":" + str(self.__proxy_port))
        ...
        if self.__connect_timeout != 0:
            self.__curl_context.setopt(pycurl.CONNECTTIMEOUT,
                                       self.__connect_timeout)
        else:
            self.__curl_context.setopt(pycurl.CONNECTTIMEOUT, 10)

        if self.__timeout != 0:
            self.__curl_context.setopt(pycurl.TIMEOUT, self.__timeout)
        else:
            self.__curl_context.setopt(pycurl.TIMEOUT, 60)
