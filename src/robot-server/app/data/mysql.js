...
queryHero = function (client, limit, offset, cb) {
  var users = [];
  var sql = "SELECT id,name,channel FROM Account where name like 'test%' and id<14999 limit ? offset ? ";
  ...
  client.query(sql, args, function selectCb(error, results, fields) {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      var user = { uid: results[i]['id'], token: token, username: results[i]['name'], passwd: '123456' };
      users.push(user);
    };
    cb(null, users);
  });
